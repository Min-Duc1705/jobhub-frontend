...
import type { CompanyBody } from '../../../types/company'
import { createCompanyApi, uploadCompanyImageApi } from '../../../services/company-service'
...
const CreateCompanyModal = ({ open, onOpenChange, onSuccess }: Props) => {
  ...
  const handleOk = async () => {
    try {
      ...
      await createCompanyApi({
        ...
        activityImages: activityFileList.length ? activityFileList.map(f => f.url as string) : undefined,
      })
      notification.success({ message: 'Thành công', description: 'Tạo công ty thành công', duration: 2 })
      form.resetFields()
      setDesc('')
      setLogoFileList([])
      setCoverFileList([])
      ...
      notification.error({ message: 'Thất bại', description: err?.response?.data?.message || 'Tạo công ty thất bại', duration: 3 })
